...
class sjf{
public:
	int process_no,arrival_time,burst_time,index;
};
int find_index(int time,vector<sjf>&process){
	int ind=-1,n=process.size(),max_time=INT_MAX;
	for(int i=0;i<n;++i){
		if(process[i].arrival_time<=time and process[i].burst_time<max_time){
			ind=i;
			max_time=process[i].burst_time;
		}
	}
	return ind;
}
...
int main(){
	...
	for(int i=0;i<n;++i){
		cin>>process[i].process_no>>process[i].arrival_time>>process[i].burst_time;
		process[i].index=i;
	}
	temp=process;
	for(auto &i:process) cout<<i.process_no<<" "<<i.arrival_time<<" "<<i.burst_time<<endl;
	int time=0;
